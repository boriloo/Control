    ...
    return (
        ...
        fixed z-100 flex-1 flex justify-center items-center w-full h-screen transition-all duration-500 cursor-pointer`}>
            <div className={`${isFullsceen ? 'max-w-full max-h-full' : 'rounded-lg max-w-[1200px] max-h-[700px]'} ${config.currentStatus === "open" ? 'scale-100' : 'scale-0 '} 
                cursor-default origin-bottom relative transition-all duration-300 flex flex-col bg-zinc-900 w-full h-full overflow-y-auto`}>
                ...
                <div className="flex flex-row w-full h-full">
                    <div className="group bg-zinc-950 h-auto flex flex-col gap-2 p-4">
                        <div className="flex flex-row w-full items-center cursor-pointer rounded-md p-2 gap-0 transition-all hover:bg-zinc-800 hover:gap-1">
                            <User />
